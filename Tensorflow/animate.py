...
from PIL import Image  # Added: Import Pillow's Image module
import square
import core
import config
...
def main():
    ...
    # Added: Initialize frame collection list
    frames = []
    ...
    for istep in range(config.EVALUATE_STEPS):
        ...
        s_np_ori, g_np_ori = square.square_data(args.num_agents, config.DIST_MIN_THRES * 1.5)
        ...
        if args.vis:
            ...
            for j in range(max(len(s_np_ours), len(s_np_lqr))):
                ...
                # Added: Capture the frame for "Ours" subplot
                fig.canvas.draw()
                img = np.frombuffer(fig.canvas.tostring_rgb(), dtype=np.uint8)
                img = img.reshape(fig.canvas.get_width_height()[::-1] + (3,))
                frames.append(img)
                print(f"Captured frame {len(frames)} with shape {img.shape}")

                # Optionally, save the first frame for verification
                if len(frames) == 1:
                    plt.imsave('test_frame.png', img)
                    print("Saved test_frame.png for verification.")

                # Plot "LQR" in subplot(122) without capturing frames
                plt.subplot(122)
        ...
        # Added: Compile and save the GIF after all evaluation steps
        if args.vis and frames:
            # Convert frames to PIL Images and save as GIF
            pil_frames = [Image.fromarray(frame) for frame in frames]
            
            # Save the GIF
            pil_frames[0].save('model_evaluation_video.gif',
                               save_all=True,
                               append_images=pil_frames[1:],
                               duration=50,  # Duration between frames in milliseconds
                               loop=0)  # Loop indefinitely
            print("Evaluation video saved as 'model_evaluation_video.gif'.")
