...
def load_data_from_file():
    ...
    with open(DATA_FILE_PATH, "r") as file:
        for line in file:
            ...
            states = np.fromstring(states, sep=',').reshape(-1, 4)
            goals = np.fromstring(goals, sep=',').reshape(-1, 2)
...
def build_optimizer(loss):
    ...
    # Print initial gradients for reference
    # for grad, var in grad_pairs:
    #     print(f"Initial Gradient for {var.name} (TensorFlow): {grad}")
...
def build_training_graph(num_agents):
    ...
    # loss_safe is for h(s) >=0, s in safe set
    ...
    #print(f"Weight Loss (TensorFlow): {weight_loss}")
...
def main():
    ...
    with tf.Session() as sess:
        ...
        for istep in range(config.TRAIN_STEPS):
            ...
            total_loss = sess.run(loss, feed_dict={s: s_np, g: g_np})
